...
(function($, window, document, undefined) {
    ...
    $.fn.cubeportfolio.Constructor.prototype._responsiveLayout = function() {
        ...
        if (t.options.ratioAuto) {
            ...
            $.each(t.blocksObj, function(index, item) {
                if (!$.data(item.el[0], 'cbp-wxh')) {
                    $.data(item.el[0], 'cbp-wxh', {
            ...
            $.each(t.blocksObj, function(index, item) {
                var data = $.data(item.el[0], 'cbp-wxh'),
                    cssObj = {
                        width: itemWidth,
                        height: parseInt(data.height * (itemWidth / data.width), 10) // @todo - verificat
                    };
...
(function($, window, document, undefined) {
    ...
    $.fn.cubeportfolio.options.ratioAuto = false;

    function AutoRatio(parent) {
        var t = this;

        t.parent = parent;

        parent._registerEvent('initStartWrite', function() {
            if (parent.options.ratioAuto) {
                parent.$obj.addClass('cbp-ratio-auto');
            }
        });

    }

    AutoRatio.prototype.destroy = function() {
        this.parent.$obj.removeClass('cbp-ratio-auto');
    };

    CubePortfolio.Plugins.AutoRatio = function(parent) {
        return new AutoRatio(parent);
    };
...
(function($, window, document, undefined) {
    ...
    $.fn.cubeportfolio.initCBP = function(id, options) {
         ...
         *********************************/
        $('#cbpw-loadMore' + id).on('click', '.cbp-l-loadMore-link', function(e) {
            var self = $(this),
                clicks, displayItemsLoadMore;

            e.preventDefault();

            if (self.hasClass('cbp-l-loadMore-stop')) {
                return;
            }

            var gridData = gridContainer.data('cubeportfolio');

            displayItemsLoadMore = parseInt(gridData.options.displayItemsLoadMore, 10);
            clicks = $.data(self[0], 'numberOfClicks');
            clicks = clicks ? parseInt(clicks, 10) : 0;
            self.addClass('cbp-l-loadMore-loading');

            var id = gridData.wrapper.attr('id');
            if (!id) {
                id = gridData.$obj.attr('id');
            }
            id = id.replace('cbpw-grid', '');
            ...
            $.ajax({
                ...
                gridContainer.cubeportfolio('appendItems', items, function() {
                    self.removeClass('cbp-l-loadMore-loading');
                    if (data.itemsRemain <= 0) {
                        self.addClass('cbp-l-loadMore-stop');
                    }
                });
